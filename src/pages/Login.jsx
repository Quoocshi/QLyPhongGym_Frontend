...
const Login = () => {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      if (!credential) {
        const msg = 'Không nhận được credential từ Google';
        setError(msg);
      ...
      addToast({ message: 'Đăng nhập Google thành công', type: 'success' });
      goAfterLogin();
    } catch (err) {
      console.error('Google login error:', err);
      const message = err?.response?.data?.error || err?.message || 'Đăng nhập Google thất bại';
      setError(message);
      addToast({ message, type: 'error' });
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-orange-50 via-white to-orange-50 relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24 w-full lg:w-1/2 relative z-10">
        ...
        <Link
          to="/"
          className="absolute top-8 left-8 flex items-center text-gray-600 hover:text-primary transition-all duration-300 font-medium group"
        >
        ...
        <div className="mx-auto w-full max-w-md opacity-0 animate-fade-in-up">
          <div className="text-center mb-10">
            <Link
              to="/"
              className="inline-flex justify-center mb-6 transform hover:scale-110 transition-transform duration-300"
            >
            ...
            <h2 className="text-4xl font-extrabold text-gray-900 mb-2">Chào mừng trở lại! 👋</h2>
            <p className="text-gray-600">Đăng nhập để tiếp tục hành trình của bạn</p>
          </div>

          {error && (
            <div
              className="bg-red-50 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded-lg mb-6 text-sm shadow-md animate-shake"
              role="alert"
            >
          ...
          <div className="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-gray-100">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="transform transition-all duration-300 hover:scale-[1.02]">
                ...
                />
                {errors.username && (
                  <p className="text-red-500 text-xs mt-2 animate-fade-in">{errors.username.message}</p>
                )}
              </div>
              ...
              <div className="transform transition-all duration-300 hover:scale-[1.02]">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <a
                    href="#"
                    className="text-sm font-medium text-primary hover:text-orange-700 hover:underline transition-all duration-300"
                ...
                />
                {errors.password && (
                  <p className="text-red-500 text-xs mt-2 animate-fade-in">{errors.password.message}</p>
                )}
              </div>
            ...
            <div className="mt-8">
              ...
              <div className="mt-6 flex justify-center transform hover:scale-105 transition-transform duration-300">
                <GoogleLogin onSuccess={handleGoogleSuccess} onError={handleGoogleError} shape="pill" size="large" />
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-600">
              ...
              <Link
                to="/register"
                className="font-bold text-primary hover:text-orange-700 hover:underline transition-all duration-300"
              >
                Đăng ký ngay →
              </Link>
            </p>
      ...
      <div className="hidden lg:block relative w-0 flex-1 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-orange-500 to-orange-700">
          <img
            ...
            src="https://images.unsplash.com/photo-1540497077202-7c8a33801524?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80"
            alt="Gym background"
          />
        </div>
